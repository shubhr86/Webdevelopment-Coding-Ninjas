  ...
  async addExpenseAndUpdateTagTransaction(addParams, updateParams) {
    ...
    try {
      await session.withTransaction(async () => {
        const insertedId = await this.addExpenseWithTransaction(expenseToCreate, session);
  
        // Update the tag in the newly created expense
        await this.updateTagInExpenseWithTransaction(insertedId, oldTag, newTag, session);
      });
  
      session.endSession();
      return "Expense added and tag updated successfully within a transaction.";
    } catch (err) {
      session.endSession();
     //console.log(err);
      throw new Error("Error performing operations within a transaction.");
    }
  }
